...
bool TiXmlBase::considerEnterForSpace	= false;
...
bool TiXmlDocument::LoadStr(const std::string& xmlstr )
{
	// Delete the existing data:
	Clear();
...
void TiXmlAttribute::SetBoolValue( bool value )
{
	char buf [64];
	if(value)
		sprintf (buf, "true");
...
const bool TiXmlAttribute::BoolValue() const
{
	return (stricmp(value.c_str (),"true") == 0?true:false);
}
