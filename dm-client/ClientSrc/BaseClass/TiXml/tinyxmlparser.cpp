...
const char* TiXmlBase::SkipWhiteSpace( const char* p )
{
	...
	while ( p && *p )
	{
		if ( isspace( *(const unsigned char*)p )
			||(considerEnterForSpace
				&&( *p == '\n' || *p =='\r' ))
    ...
    *text = "";
	...
	else
	{
				...
				&& !StringEqual( p, endTag, caseInsensitive ) )
		{
			...
			else if ( isspace( *(const unsigned char*)p ) )
			{
				whitespace = true;
				++p;
			}
...
const char* TiXmlAttribute::Parse( const char* p )
{
	...
	else
	{
		...
		value = "";
		while (    p && *p										// existence
				&& !isspace( *(const unsigned char*)p )			// whitespace
				&&!(IsEnterForSpace() && (*p == '\n' || *p == '\r'))
				&& *p != '/' && *p != '>' )						// tag end
		{
			value += *p;
...
//			p += 10;
		...
		else
		{
			...
			while( p && *p && *p != '>' && !isspace( *(const unsigned char*)p ) )
				++p;
		}
	}
	return 0;
...
bool TiXmlText::Blank() const
{
	for ( unsigned i=0; i<value.length(); i++ )
		if ( !isspace( (const unsigned char)value[i] ) )
			return false;
	return true;
}
