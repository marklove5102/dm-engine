...
int CNetData::AddFloat(float fFloat)//08-10-06 liujunhui add: 增加解析float
{
...
int CNetData::DelFloat(float &fFloat)//08-10-06 liujunhui add: 增加解析float
{
	if(m_iPos + (int)sizeof(fFloat) > m_iSize)
		return -1;
...
int CNetData::AddDouble( double dbDouble )
{
...
int CNetData::DelDouble( double &dbDouble )
{
	if(m_iPos + (int)sizeof(dbDouble) > m_iSize)
		return -1;
