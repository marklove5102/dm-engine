...
#include <shellapi.h>
#include "Global.h"
#include "Global/Interface/GraphicInterface.h"  //为了显示显卡信息
...
#define _MAX_RESULT _MAX_PATH + 1024
...
//
TCHAR ExceptionReport::m_szDumpFileName[MAX_PATH];
TCHAR ExceptionReport::m_szLogFileName[MAX_PATH];
TCHAR ExceptionReport::m_szCrashReporter[MAX_PATH];
...
ExceptionReport::ExceptionReport( )   // Constructor
{
	...
	LPTSTR pszFileName = _tcsrchr(m_szLogFileName, _T('\\'));
	if (pszFileName)
	{
		pszFileName++;
		*pszFileName = _T('\0');
	...
	TCHAR* szTime = g_Timer.GetDateTime("%Y%m%d%H%M%S");

	// Create full path for DUMP file
	sprintf_s(m_szDumpFileName,"%sException%s.dmp",m_szLogFileName,szTime);

	// Create full path for crashreporter
	_tcsncat(m_szCrashReporter,m_szLogFileName,MAX_PATH-1);
	_tcsncat(m_szCrashReporter,_T("CrashReporter.exe"),MAX_PATH - 1);

	sprintf_s(m_szLogFileName,"%sException%s.prt",m_szLogFileName,szTime);
...
//==============================================================
void ExceptionReport::SetLogFileName( PTSTR pszLogFileName )
{
...
{
	//1.Generate local report - Exception.prt in module directory
	sprintf_s(m_szLogFileName,"Exception%d.prt", GetTickCount());
	...
	//2.Generate mindump file report - upload with outer program - Crashreporter.exe
	//We can use WINDBG to anylisis this minidump file,with PDB File backuped earlier.
	LONG lRetValue = GenerateDumpReport(pExceptionInfo);

#ifndef _DEBUG
	// Exit the process only in release builds, so that in debug builds the exception is passed to a possible installed debugger
    ...
    PEXCEPTION_POINTERS pExceptionInfo )
{
	...
	// 版本信息
	_tprintf(_T("\r\nversion: %s"),g_strVersion);
	_tprintf(_T("\r\ndxver: %x"),g_iDXVersion);
	_tprintf(_T("\r\nruntime: %5d seconds"),g_Timer.GetMicroseconds() / 1000);
...
// 获取硬件消息
const char* GetHwInformation()
{
	HKEY hKey;

	static char strResult[4096] = {0};
	strcpy(strResult,"");
	...
	sprintf(strResult,"CPU: %dMHz,%s,%s\r\n",(int)dwMhz,strVendor,strCpu);
	...
	GetVersionEx(&osvi);
	char stmp[1024];
	sprintf(stmp,"OS: %u,%u,%u,%u\r\n",osvi.dwMajorVersion,osvi.dwMinorVersion,osvi.dwBuildNumber,osvi.dwPlatformId);
	strcat(strResult,stmp);

	// 得到内存数据
	MEMORYSTATUS ms;
	GlobalMemoryStatus(&ms);
	sprintf(stmp,"mem(F/T): %d%%,",ms.dwMemoryLoad);
	strcat(strResult,stmp);

	if( ms.dwAvailPhys >= (1024*1024) )
		sprintf(stmp,"%dM/%dM,",ms.dwAvailPhys/1024/1024,ms.dwTotalPhys/1024/1024);
	else
		sprintf(stmp,"%dk/%dM,",ms.dwAvailPhys/1024,ms.dwTotalPhys/1024/1024);
	strcat(strResult,stmp);

	if( ms.dwAvailPageFile >= (1024*1024) )
		sprintf(stmp,"PF:%dM/%dM,",ms.dwAvailPageFile/1024/1024,ms.dwTotalPageFile/1024/1024);
	else
		sprintf(stmp,"PF:%dk/%dM,",ms.dwAvailPageFile/1024,ms.dwTotalPageFile/1024/1024);
	strcat(strResult,stmp);

	if( ms.dwAvailVirtual >= (1024*1024) )
		sprintf(stmp,"V:%dM/%dM\r\n",ms.dwAvailVirtual/1024/1024,ms.dwTotalVirtual/1024/1024);
	else
		sprintf(stmp,"V:%dk/%dM\r\n",ms.dwAvailVirtual/1024,ms.dwTotalVirtual/1024/1024);
	strcat(strResult,stmp);

	// 获得显卡驱动
	strcat(strResult,"Viedo: ");
	//g_Gfx.GetAdapterInfo(stmp);
	strcat(strResult,stmp);

	return strResult;
}

HMODULE ExceptionReport::GetDebugHelperDll(FARPROC* ppfnMiniDumpWriteDump)
{
	*ppfnMiniDumpWriteDump = NULL;
	...
	if (hDll)
	{
		*ppfnMiniDumpWriteDump = GetProcAddress(hDll, "MiniDumpWriteDump");
		//如果(*ppfnMiniDumpWriteDump == NULL) //DBGHelp.dll的版本太低了，无法生成dump文件
	}
	return hDll;
}

LONG ExceptionReport::GenerateDumpReport(PEXCEPTION_POINTERS pExceptionInfo)
{
	LONG lRetValue = EXCEPTION_CONTINUE_SEARCH;
	TCHAR szResult[_MAX_RESULT] = {0};
	...
	if (hDll)
	{
		...
		{
			sprintf_s(m_szDumpFileName,"Exception_%d_%d.dmp", GetTickCount(), rand()%1000);

			HANDLE hFile = CreateFile(m_szDumpFileName, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
			if (hFile != INVALID_HANDLE_VALUE)
			{
				...
				if (bOK)
				{
					...
					_sntprintf(szResult,_MAX_RESULT , _T("Saved dump file to \"%s\".\r\n\r\nPlease send this file together with a detailed bug report to xcbcrash_report@shandagames.com !\r\n\r\nThank you for helping to improve XCB."), m_szDumpFileName);
					lRetValue = EXCEPTION_EXECUTE_HANDLER;

					hInstCrashReporter = ::ShellExecute(NULL, _T("open"), m_szCrashReporter, m_szDumpFileName, NULL, SW_SHOW);
				...
				{
					// Do *NOT* localize that string (in fact, do not use MFC to load it)!
					_sntprintf(szResult, _MAX_RESULT, _T("Failed to save dump file to \"%s\".\r\n\r\nError: %u"), m_szDumpFileName, GetLastError());
				}
				CloseHandle(hFile);
			}
			else
			{
				// Do *NOT* localize that string (in fact, do not use MFC to load it)!
				_sntprintf(szResult, _MAX_RESULT, _T("Failed to create dump file \"%s\".\r\n\r\nError: %u"), m_szDumpFileName, GetLastError());
			}
		}
		FreeLibrary(hDll);
		hDll = NULL;
