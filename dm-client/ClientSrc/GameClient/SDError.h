...
用户报告具体的处理方法见!!!link 用户报告提交收集分析流程.htm 用户报告提交收集分析流程!!!。
!externpage doc\用户报告提交收集分析流程.htm jpg
...
// 版本号
const SSGDPVersion	SDERROR_VERSION = {0,3,0,3};
...
/*!
!macro SDERROR_RUNTIMEDATAID_
运行时信息ID。
!inc SDError.h
*/
#define SDERROR_RUNTIMEEVENTID_NETDELAY		1				//! 网络延迟
#define SDERROR_RUNTIMEEVENTID_SERVERDELAY	2				//! 服务器延迟
#define SDERROR_RUNTIMEEVENTID_LOGINDELAY	3				//! 登录延迟
#define SDERROR_RUNTIMEEVENTID_FPS			4				//! 渲染帧速统计
#define SDERROR_RUNTIMEEVENTID_NETRECV		5				//! 接收网络数据统计
#define SDERROR_RUNTIMEEVENTID_NETSEND		6				//! 发送网络数据统计
#define SDERROR_RUNTIMEEVENTID_DISCONNET	7				//! 发生非正常断线
#define SDERROR_RUNTIMEEVENTID_CHEAT		8				//! 使用外挂

#define SDERROR_RUNTIMEEVENTID_VERSION      9               //! 版本信息
#define SDERROR_RUNTEIMEVENTID_INSTALLVERSION   10          //! 安装版本
#define SDERROR_RUNTIMEEVENTID_GAMENAME     11              //! 游戏名称
#define SDEEROR_RUNTIMEEVENTID_GAMESECTION  12              //! 游戏大区
#define SDERROR_RUNTIMEEVENTID_GAMEGROUP    13              //! 游戏组
#define SDERROR_RUNTIMEEVENTID_USERID       14              //! 玩家ID
#define SDERROR_RUNTIMEEVENTID_MACHINAID    15              //! 机器ID
#define SDERROR_RUNTIMEEVENTID_USERCONFIGURATION   16       //! 用户配置
#define SDERROR_RUNTIMEEVENTID_SETURL       17              //! 用户设置上传服务器的URL
#define SDERROR_RUNTIMEEVENTID_TOTALTIME    18              //! 统计组件被使用的时间，单位为秒
#define SDERROR_RUNTIMEEVENTID_USER         1000            //! 用户自定义 


/*!
!macro SDERROR_RUNTIMEDATATYPE_
信息的类型，表示为数据型还是文本型
*/
#define SDERROR_RUNTIMEDATATYPE_INT        1                //!表示数据为INT型
#define SDERROR_RUNTIMEDATATYPE_DWORD      2                //!表示数据为DWORD型
#define SDERROR_RUNTIMEDATATYPE_FLOAT      3                //!表示数据为FLOAT型
#define SDERROR_RUNTIMEDATATYPE_DOUBLE     4                //!表示数据为DOUBLE型
#define SDERROR_RUNTIMEDATATYPE_CHAR       5				//!表示数据为字符串
...
!inc SDError.h
*/
#define SDERROR_RUNTIMEEVENTTYPE_BEGIN		1				//! 动作开始
#define SDERROR_RUNTIMEEVENTTYPE_END		2				//! 动作结束
#define SDERROR_RUNTIMEEVENTTYPE_OCCUR		3				//! 动作发生
#define SDERROR_RUNTIMEEVENTTYPE_UPDATE		4				//! 数据更新
...
第二个参数为DUMP模式，用户可以通过修改此模式来改变发送数据的模式。
!inc SDError.h
*/
typedef void (*SDERROR_SENDERROR_CB)(char *,unsigned long &);
...
public:
	...
	*/
	...
	/*!func
	收集客户端运行时信息。
	!param [in] 运行时信息ID，见SDERROR_RUNTIMEDATAID_。
	!param [in] 运行时信息类型，见SDERROR_RUNTIMEDATATYPE_。
	!param [in] 运行时信息。
	!return 是否成功。
	*/
	virtual bool			SDAPI	RuntimeData			(unsigned long dwEventID, unsigned long dwEventType, unsigned long dwDataType, unsigned long dwData = 0) = 0;

	/*!func
	设置收集运行时数据的服务器地址，这些信息可能直接从SGDP.dat取得，也可能由用户程序主动设置。
	!param [in] 游戏名称。
	!param [in] 运行时信息收集服务器地址，例如“http://runtimedata.sdo.com/index.php”。
	*/
	//virtual void			SDAPI	SetRuntimeDataServer(const char * strServer = 0) = 0;
...
//void SDAPI GetVersionInfo( char* pszUrl,  char* pszGamename,  char* pszVersion);
//=============================================================================================================================
/*!func
...
extern SGDP::ISDError* g_pSDError;
